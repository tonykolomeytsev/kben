...
class UserCredentialsTypeAdapter : TypeAdapter<UserCredentials>() {
    ...
    override fun fromBencode(value: BencodeElement, context: DeserializationContext, typeHolder: TypeHolder): UserCredentials {
        value as BencodeElement.BencodeDictionary
        return UserCredentials(
            username = context.fromBencode(value.entries["username"]!!, TypeHolder.Simple(String::class)),
            password = context.fromBencode(value.entries["password"]!!, TypeHolder.Simple(String::class))
        )
    }
